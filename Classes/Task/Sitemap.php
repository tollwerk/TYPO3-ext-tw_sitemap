...
class Sitemap extends AbstractTask
{
    ...
    protected function _generateSitemap(\Tollwerk\TwSitemap\Domain\Model\Sitemap $sitemap)
    {
        ...
        $sitemapEntriesResult = $db->exec_SELECTquery('GROUP_CONCAT(loc ORDER BY position ASC) AS loc,MAX(lastmod) AS lastmod,MIN(changefreq) AS changefreq,MAX(priority) as priority,GROUP_CONCAT(language ORDER BY position ASC) AS language',
            'tx_twsitemap_domain_model_entry',
            'domain='.$db->fullQuoteStr($sitemapDomain, 'tx_twsitemap_domain_model_entry').' AND deleted=0',
            'CONCAT(origin, "~", source)', 'priority DESC, lastmod DESC');
        if ($sitemapEntriesResult && $db->sql_num_rows($sitemapEntriesResult)) {
            ...
            while ($sitemapEntryRecord = $db->sql_fetch_assoc($sitemapEntriesResult)) {
                ...
                $sitemapEntry = '<url><loc>'.htmlspecialchars($sitemapSchemePath.$locs[0]).'</loc><lastmod>'.date('Y-m-d',
                        $sitemapEntryRecord['lastmod']).'T'.date('H:i:s',
                        $sitemapEntryRecord['lastmod']).'Z</lastmod><changefreq>'.\Tollwerk\TwSitemap\Domain\Model\Entry::$changefreqs[$sitemapEntryRecord['changefreq']].'</changefreq><priority>'.number_format($sitemapEntryRecord['priority'],
                        1).'</priority>';
                ...
                $currentSitemapSize += $sitemapGzip ? fwrite($currentSitemapResource,
                    $sitemapEntry) : gzwrite($currentSitemapResource, $sitemapEntry);
                ++$sitemapURLs;
            }
            ...
            if (count($sitemapFiles) > 1) {
                ...
                $success = rename($sitemapTmpDirectory.DIRECTORY_SEPARATOR.$sitemapFiles[0],
